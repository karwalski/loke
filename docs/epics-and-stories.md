...
> **F8.1–F8.4 cannot currently be built, let alone be Done.** The bindings exist, but `std.mdns` declares its only parameter type as `$service_record` and `std.tls` declares every `TlsConfig` field with an underscore — and toke rejects underscores in identifiers. A type whose name cannot be written cannot be constructed, so `companion/discovery.tk` and `companion/channel.tk` are **blocked upstream**, not merely unverified. See the fourth upstream ask in F10.11. The same defect blocks `std.infer`'s options struct (epic F2) and `std.mlx` entirely, which is the claim LC1.3 exists to check. These four rows stay in RV1, but as **reopen** candidates rather than re-verify ones.
...
| F10.7 | M | **In progress** — and the target is **209 of 217, not 217**. Eight modules (`companion/channel`, `companion/discovery`, `companion/executor`, `models/infer`, `models/mlx`, `models/streaming` and their dependents) import a stdlib module whose identifiers toke cannot name — see the fourth upstream ask in F10.11 — so they cannot compile at any effort on loke's side. The other 40 non-emitting modules are fixable here. Measured 2026-09-20 — re-scoped 2026-09-20. The "74 type errors" figure was measured against stale interfaces and is wrong. Measured now: 58 of 199 modules compile, 291 errors of which **226 are E2030 cascade**, leaving 65 real errors in 19 files. The classes are split out as F10.14–F10.24 | **Finish the migration** — the 74 remaining genuine type errors, and whatever the expression-`if` part of the v0.4 change requires. Boolean operators are already migrated (53 `&&`, 74 `\|\|`, zero `and(`/`or(` call sites) and the equality split is done, so this is the residual. |
...
| F10.11 | S | | **File the genuine gaps upstream** — after F10.10's verification the list is short: the document and image library asks in [`toke-libraries-required.md`](toke-libraries-required.md), and the three toke documents that still state "toke has no comment syntax" after `(* *)` shipped (`companion-file-spec.md`, `companion-fidelity-methodology.md`, `conventions.md`). Two further asks were found on 2026-09-20, both tool hazards rather than niceties. **(1) `toke --lint --fix` deletes imports that are needed.** The `unused-import` rule looks only at alias usage (`t.foo`) and ignores **type** references, so it removes `i=t:shared.types;` from a file whose only use of that module is `$lokeerr`. Applied here it dropped 197 imports across 119 files and produced 265 `identifier 'lokeerr' is not declared` errors. Worth filing with the observation that this is the same class of hazard F10.6 removed loke's own destructive rewriter for — a `--fix` flag that breaks a correct file is worse than no `--fix`. **(2) `http.post` is declared twice in `http.tki`**, once as a `route` taking `(str, funcdecl)` and once as a `func` taking `(httpclient, str, [byte], str)`. toke resolves the route, so **the function is unreachable from user code** — no argument list satisfies it, confirmed by migrating call sites correctly to the four-parameter form and getting the same error. `http.postheaders` and `http.postjson` are the workarounds. **(3) The regex engine silently ignores unsupported syntax.** `\s`, `\S`, non-greedy `*?` and the inline flag `(?i)` all return no match rather than raising, so a pattern using them compiles, runs and quietly does nothing — which is how `privacy/content.tk`'s script-stripping sanitiser passed review for months. Either support them or reject them at compile time; silence is the worst option. **(4) Six stdlib interfaces declare identifiers the language rejects — the most
consequential of these asks.** toke refuses an underscore in an identifier
("contains underscore (v0.2 syntax)"), but the shipped `.tki` files declare:

| module | unnameable identifiers | what it blocks |
|---|---|---|
| `std.mdns` | `$service_record` — the **only** parameter type of `mdns.advertise` | mDNS pairing entirely (`companion/discovery.tk`, epic F8.1) |
| `std.tls` | `cert_pem`, `key_pem`, `peer_cert_pem`, `require_mutual` — every `TlsConfig` field | TLS 1.3 mutual auth with pinning (`companion/channel.tk`, F8.2) |
| `std.infer` | `infer_opts`, `model_handle`, `n_gpu_layers`, `n_threads`, `prefetch_layers`, `ram_ceiling_gb` | the local inference options struct (`models/infer.tk`, epic F2) |
| `std.mlx` | `mlx_model` | MLX entirely — which is the claim LC1.3 exists to verify |
| `std.http` | `pool_size`, `timeout_ms` | per-client timeouts (see ask 5) |
| `std.analytics` | `rolling_mean` | that function |

A type whose name cannot be written cannot be constructed, and a struct field that
cannot be named cannot be read or set. So these modules are not merely awkward from
toke source — they are **unreachable**. This is one fix upstream (rename to the
identifier grammar the compiler accepts, or accept underscores) and it unblocks five
loke features at once.

**(5) String interpolation of `str.replacere`'s return prints a pointer** — `str.len` on the same value is correct, so the value is fine and the interpolation path mistypes it. Cross-reference into toke's own progress tracker so they are scheduled rather than noted. |
...
| F10.16 | M | **Partial** — the `http.post` and `json.obj` halves are done and both turned up something the story did not anticipate. **`std.http.post` is not callable at all**: `http.tki` declares it twice, once as a `route` taking `(str, funcdecl)` and once as a `func` taking `(httpclient, str, [byte], str)`, and toke resolves the route — so no argument list satisfies it. That escalates the F10.11 upstream ask from "misleading diagnostic" to "the function is unreachable". Worked around with `http.postheaders`, which is not double-declared and preserves the existing `$ok`/`$httperr` handling; `http.postjson` also works. `json.obj` takes a raw JSON string rather than key/value pairs, so the four bodies in `models/ollama.tk` are built with interpolation — the canonical form, and correctly escaped via `json.enc` on each value. `str.between` and the regex-flags argument are now done too, and that half found something worse than an arity change: **toke's regex engine supports neither `\s`/`\S` nor non-greedy `*?`** — verified by running probes, both silently return no match rather than erroring. `privacy/content.tk`'s script/style stripping depended on both, so it could never have worked as a regex and is now an index walk (`shared.types.strstripregion`), which also leaves an unclosed tag alone instead of eating the rest of the document. `(?i)` is ignored too, so case-insensitivity is either a spelled-out character class or a lowercased input. Still open: `http.gettimeout`, `std.process.exec`, `std.tls.tlsconfig`, `std.infer.generate` | **One stdlib compatibility shim, `shared.strcompat`** — six drifts, none of them mechanical. `str.between` does not exist at all (needs an `indexof`+`slice` helper; supersedes F10.12b). `replacere`/`countre`/`containsre` lost their trailing flags argument, so each `"gi"`/`"g"`/`"i"` has to be folded into the pattern — a per-site regex judgement. `str.format` reverses its arguments and drops varargs (9 sites, 4 files). `json.obj` takes a raw JSON string, not key/value pairs (9 sites, 5 files). `json.try*` each need a second argument (5 sites in `optimiser/toon.tk`). `str.pad`/`padleft`/`padright` take `(s, width, padchar)`; 3 sites pass 2 and are latent. |
...
| F10.24 | S | **Done** — and both were the same anti-pattern, not two unrelated typos. `pages/api/settings.tk:100` and `governance/dsar.tk:143` each built JSON with a nested `str.concat` chain — nine deep in dsar's case — and each had one closing parenthesis too many, so **neither file has ever parsed**. Interpolation fixes the parse error and the `string-concat-chain` lint violation together. dsar's payload also referenced five export tables, which is only checkable once the file parses at all | **Two parse errors** — unbalanced parentheses in `packages/browser/pages/api/settings.tk:100` (one `)` too many in a nested `str.concat`) and `packages/core/src/governance/dsar.tk:143`. Pre-existing, nothing to do with the migration. |
...
| LS1.1a | L | | **Thirteen accumulations that silently do nothing** — The most consequential defect found in the whole migration, and **no lint rule reports it**. Each of these reads as an append or an increment and is a no-op `==` comparison in statement position:<br><br>**`privacy/placeholder.tk:41`** — `map.entries==str.arraypush(map.entries;entry)`. **The placeholder map never stores an entry.** Everything downstream of it — restoration, the mapping shown in the privacy review, PL1's collision work — rests on a table that is always empty.<br>**`governance/policy.tk:85,87`** — `controls==@(controls;"human-visible-preview";"cost-confirmation";…)`. **The controls a policy decision requires are never attached to it**, so a decision that should demand preview and cost confirmation demands nothing.<br>**`agents/registry.tk:42,188`** — `reg.agents==@(reg.agents;agent)` and `reg.runs==@(reg.runs;run)`. **Registering an agent never adds it to the registry**, and no run is ever recorded.<br>**`agents/sandbox.tk:139,158` and `agents/executor.tk:41,77`** — `sandbox.violations==@(sandbox.violations;msg)`. **No sandbox or executor violation is ever recorded.** With `recordcost` (also a no-op, fixed) that means the sandbox tracked neither cost nor violations — the two things it exists to track.<br>**`governance/use_cases.tk:126`** — use cases are never registered.<br>**`mcp/discovery.tk:122`** — discovered MCP servers are never added to the list.<br>**`memory/search.tk:93`** — keyword relevance never accumulates, so multi-keyword search ranks by insertion order.<br>**`moke/src/dashboard_flow.tk:167`** — appends a computed value into a card label; this one failing is a mercy, since that is the display-channel smuggling NC1.5 strips.<br><br>**Root cause.** All thirteen try to mutate a struct reached through a parameter. toke has value semantics: a callee cannot write to its caller's struct, so these were wrong before the `=`/`==` migration and it only changed how they fail. The fix per site is for the function to return the updated value and every caller to use it — not a one-character edit.<br><br>**What this means for the backlog.** Several of these sit under stories marked **Done**: the agent registry and sandbox (F7/GA-series), policy control attachment (GA1.8), MCP discovery, and the placeholder store. They belong in **RV1** as reopen candidates, and the placeholder one should be checked before any PL1 measurement is taken, because a recall figure computed against an always-empty map measures nothing.<br><br>**How to find them.** `([A-Za-z_][A-Za-z0-9_.]*)\s*==\s*(.*\1.*)` anchored to statement position. Note BSD `grep -E` silently matches nothing for a backreference, so use Python; my first sweep with grep reported zero and I believed it |
| LS1.1b | M | **117 of 135 done** | **`x=@(x;item)` builds a nested pair, it does not append** — Verified by running: `@(a;4)` where `a` has three elements produces an array of length **2**, while `a.push(4)` produces length 4. So `@(arr;item)` is an array **literal** containing the array and the item.<br><br>135 sites across 32 files used it as an append: agent registries and run lists, overnight job queues, pipeline step results, quota rules, policy lists, CLI column widths, discovered servers. Every one built `[[[…], item]` instead of a growing list, so each accumulation produced a malformed nested structure rather than the collection its caller expected.<br><br>This is also why so many type errors read **"expected 'array', got 'array'"** — the two arrays differ in element type because one of them is nesting.<br><br>117 converted to `.push()` mechanically. The remaining 18 are multi-item literals (`@(x;a;b)`) where the intent needs reading, and the four under LS1.1a where the target is a struct field reached through a parameter and cannot be assigned at all.<br><br>Same root cause as LS1.1a: value semantics. `@(…)` constructs, `.push()` appends, and neither mutates the receiver |
