...
| MK8.7 | S | **Done** | **Cross-dataset relationships** — Define foreign key relationships: server_id links hardware↔metrics↔incidents↔changes↔assets. app_id links performance↔servers (via deployment mapping). Add relationship metadata to dataset registry so moke can suggest joins and cross-dataset queries. |
...
| F10.10 | M | | **Consolidate the upstream gap docs** — four exist, addressed to the toke and ooke teams and filed nowhere: `ooke-gaps.md`, `docs/linker-gaps.md`, `docs/ooke-bindings-required.md` (complete) and `toke-migrate-issues.md`. **Verify before filing anything**: `docs/linker-gaps.md` is stale — 37 of the 38 symbols it lists as missing now exist in toke, so filing it as-is would mean reporting 37 already-fixed bugs and burying the one real gap. Consolidate the survivors into a single `UPSTREAM.md`. |
| F10.11 | M | | **File the genuine gaps upstream** — as issues on the toke and ooke repositories, cross-referenced into toke's own progress tracker so they are scheduled rather than noted. Includes the `<a<b` parser ambiguity (returning a comparison is indistinguishable from a return followed by a less-than), and the three toke documents that still state "toke has no comment syntax" after `(* *)` shipped. The document and image library asks are already written up in [`toke-libraries-required.md`](toke-libraries-required.md). |
...
| MK19.10 | S | | **Correct the dataset counts** — README, `features-moke.md` and `registry.tk` state three different numbers, none of which is 15. Fix all of them and add the count to the manifest so it is derived rather than asserted. |
