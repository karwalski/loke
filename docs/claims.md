...
| A13 | "The browser handler now calls the core privacy pipeline directly (GA1.1-GA1.8 completed) — all browser traffic flows through the full detection stack, response restoration, kill switch check, audit logging, token optimisation, semantic cache, intelligent routing, and governance policy evaluation" | `docs/features-loke.md:52`; implied by `README.md:225`, `docs/architecture.md:96-133` | `absent` | `packages/browser/pages/api/pipeline.tk:9-66` is the route registered at `packages/browser/src/_handlers.tk:34-35,129`. It imports only `std.http`, `std.str`, `std.json` and `std.file`. The one browser file that does import `core.privacy.pipeline` — `packages/browser/extensions/core.tk:5` — is reached solely from `packages/browser/pages/installer.tk:7` | `tests/unit/browser/test_pipeline_handler.tk` imports the handler but asserts on its JSON shape, not on whether anonymisation occurred. GA1 needs reopening; NC1.3, AD1.6 | **None of the nine clauses holds on the browser path.** The handler calls the sidecar at `127.0.0.1:11435` over HTTP directly (`:21`, `:29`). If the sidecar does not answer, it falls through to five literal `str.contains` checks (`:36-41`) that only *increment a counter* — `anonymised` is never reassigned, so it remains the raw input — and then posts that raw text to `api.anthropic.com` (`:56`) or Ollama (`:62`) while still returning an `entities_found` count and a `sensitivity` label. There is no restore step, no kill-switch check, no audit write, no optimiser, no cache and no policy evaluation on this path |
